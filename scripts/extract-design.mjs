...
async function fetchDesignProjects() {
  ...
  await writeFile(join(rawDir, 'collection.json'), JSON.stringify(data, null, 2));
...
async function fetchProjectDetail(urlId) {
  ...
  const rawDir = join(__dirname, '_raw', 'design');
  await writeFile(join(rawDir, `${urlId}.json`), JSON.stringify(data, null, 2));
...
async function scrapeProjectContent(urlId) {
  ...
  const imageRegex =
    /https:\/\/images\.squarespace-cdn\.com\/content\/[^"'\s)]+\.(jpg|jpeg|png|gif|webp)/gi;
  ...
  if (textMatches.length > 0) {
    ...
    if (textMatches.length > 1) {
      body = textMatches.map((m) => m[1]).join('\n\n');
    }
  }

  console.log(
    `  Found ${uniqueUrls.length} unique images, ${description.length} chars description`
  );
...
async function processProject(listItem, stats) {
  ...
  try {
    ...
    const description = projectContent.description || item.excerpt || '';
    const body = projectContent.body || item.body || '';

    // Extract external links if any
    const externalUrl = item.sourceUrl || null;
